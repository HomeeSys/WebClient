...
//  Material UI
import { Stack, Grow, Box, Tooltip, Divider, Fade, Menu, MenuItem, IconButton, TableContainer, TablePagination, InputLabel, DialogTitle, DialogContent, DialogActions, Dialog, Select, Switch, Paper, FormControl, Snackbar, LinearProgress, Typography, ListItemText, Badge, Button } from '@mui/material';
...
function Requests() {
  ...
  const FetchAllStatuses = () => {
    fetch('https://localhost:6066/raports/statuses/all')
      .then(res => {
        if (!res.ok) throw new Error('Network response was not ok');
        return res.json();
      })
  ...
  const FetchRequests = React.useCallback(() => {
    ...
    if (filterDateFrom) {
      params.append('CreationDateFrom', filterDateFrom.toISOString());
    }
    if (filterDateTo) {
      params.append('CreationDateTo', filterDateTo.toISOString());
    }
    ...
    fetch(`https://localhost:6066/raports/requests/query?${params.toString()}`)
      .then(res => {
        if (!res.ok) throw new Error('Network response was not ok');
        return res.json();
      })
      .then(data => {
        setTimeout(() => {
          ...
          setTotalRequests(data.totalCount || data.total || data.length || 0);
          setAbsoluteRequestsCount(data.absoluteCount)
  ...
  const ConfirmCreateRequest_ButtonClick = async () => {
    ...
    try {
      ...
      const response = await fetch('https://localhost:6066/raports/requests', {
        method: 'POST',
        headers: {
  ...
  React.useEffect(() => {
    ...
    connection.start()
      .then(() => {
        ...
        setServerResponding(true);
        connection.on('RequestCreated', (dto) => {
          CreateSnackAlert_Handler('Request created', `New ${dto.period.name} request was created. To apply latest changes refresh collection!`);
        });
      })
      .catch((error) => {
        ...
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          >
            <Paper variant='outlined'>
              ...
              <DialogTitle sx={{ display: 'flex', alignItems: 'center', padding: 1, paddingLeft: 1.5, paddingRight: 1 }}>
                <Typography fontWeight='bold'>Create request</Typography>
                <IconButton
                  aria-label="close"
              ...
              <DialogContent sx={{ display: 'flex', flexDirection: 'column', padding: 1.5, pt: 1.5, width: 400, height: 300, minHeight: 0, position: 'relative' }}>
                  ...
                  >
                    <Box>
                      <Request
                        key={request.id}
                        id={request.id}
                        requestCreatingDate={request.requestCreationDate}
